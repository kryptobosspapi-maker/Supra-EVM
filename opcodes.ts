...
import { EVMStack, TWO_256, fromSigned, normalizeWord, toSigned } from './stack';
...
export interface ExecutionContext {
  ...
  halt: () => void;
  setReturnData: (data: Uint8Array<ArrayBufferLike>) => void;
}

function storageKey(value: bigint): string {
  return `0x${normalizeWord(value).toString(16).padStart(64, '0')}`;
}
...
function validateJumpDestination(
  bytecode: Uint8Array<ArrayBufferLike>,
  destination: number
): void {
...
const SLT: Opcode = {
  ...
  handler: (ctx) => {
    const a = toSigned(ctx.stack.pop());
    const b = toSigned(ctx.stack.pop());
    ctx.stack.push(b < a ? 1n : 0n);
  },
};
...
const NOT: Opcode = {
  ...
  handler: (ctx) => {
    const value = ctx.stack.pop();
    ctx.stack.push((TWO_256 - 1n) ^ value);
  },
};
...
const SHL: Opcode = {
  ...
  handler: (ctx) => {
    ...
    ctx.stack.push(normalizeWord(value << shift));
  },
};
...
const SAR: Opcode = {
  ...
  handler: (ctx) => {
    const shift = ctx.stack.pop();
    const value = toSigned(ctx.stack.pop());
    if (shift >= 256n) {
      ctx.stack.push(value < 0n ? TWO_256 - 1n : 0n);
      return;
    }
    ctx.stack.push(fromSigned(value >> shift));
  },
};
...
const SSTORE: Opcode = {
  ...
  handler: (ctx) => {
    const slot = ctx.stack.pop();
    const value = ctx.stack.pop();
    ctx.storage.set(storageKey(slot), normalizeWord(value));
  },
};
